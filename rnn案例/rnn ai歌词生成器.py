...
class TextGenerator(nn.Module):
    ...
    def init_hidden(self, bs):      # batch_size
        ...
        return torch.zeros(1, bs, 256).to(device)  # 迁移到GPU
...
def train():
    ...
    # 3. 初始化(神经网络)模型
    model = TextGenerator(unique_word_count).to(device)  # 迁移到GPU
    # 4. 创建数据加载器对象.
    ...
    os.makedirs('model', exist_ok=True)  # 加这一行
...
def evaluate(start_word, sentence_length):
    ...
    for i in range(sentence_length):
        # 7.1 模型预测.
        input_tensor = torch.tensor([[word_idx]]).to(device)
